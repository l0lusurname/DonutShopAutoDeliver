...
function createBot() {
  ...
  bot.on('message', (message) => {
    ...
    if (msg.includes('paid') || msg.includes('received') || msg.includes('
}
...
async function processPurchase(data) {
  ...
  console.log('\n🔍 Matching product configuration...');
  ...
  if (data.productId && CONFIG.products[data.productId]) {
    ...
    console.log('✓ Matched by ID:', matchedProductId);
  } else {
    // Try to match by product name
    for (const [productId, config] of Object.entries(CONFIG.products)) {
      console.log(`  Checking: "${config.name}" vs "${data.productName}"`);
      if (config.name.toLowerCase() === data.productName.toLowerCase()) {
        productConfig = config;
        matchedProductId = productId;
        console.log('✓ Matched by name:', matchedProductId);
        break;
      }
    }
  }
  ...
  console.log('  Amount per unit:', productConfig.amountPerUnit);
  console.log('  On purchase command:', productConfig.onPurchaseCommand);

  // Execute the on-purchase command (e.g., /afk 33)
  if (productConfig.onPurchaseCommand) {
    console.log('\n🎯 Executing on-purchase command...');
  ...
  console.log('\n💰 Payment Calculation:');
  console.log(`  ${productConfig.amountPerUnit} x ${data.quantity} = ${totalAmount}`);
  ...
  console.log(`\n✅ Payment queued: ${formattedAmount} to ${data.inGameName}`);
  console.log('===========================\n');
}
